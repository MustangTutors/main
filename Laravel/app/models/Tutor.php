...
class Tutor extends Eloquent{
    ...
    public static function getTutorsCourses($tutor_id){
        $result = DB::select('select course_id,subject,course_number,course_name from courses INNER JOIN courses_tutored ON courses_tutored.course_id = courses.course_id where courses_tutored.user_id = ?', array($tutor_id));
        return $result;
    }
